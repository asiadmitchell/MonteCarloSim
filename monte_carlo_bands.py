...
infile = open('/Users/mitcheas/Documents/MMG/Nicole Owen/Radials_band.txt', 'U')
outfile = open('/Users/mitcheas/Documents/MMG/Nicole Owen/Radials_band_out.txt', 'w')
...
for line in data:
    ...
    bands.append(line[0])
    if line[1] != '':
        DEB.append(int(line[1]))
    else:
        DEB.append(0)
    if line[2] != '':
        MMC.append(int(line[2]))
    else:
        MMC.append(0)
    if line[3] != '':
        ...
        Total.append(0)        
numbands = len(bands)
RANGE = range(0, numbands)
distribution = range(0, numbands)
for i in distribution:
    distribution[i] = 1/numbands
##DEBsum = sum(DEB)
##MMCsum = sum(MMC)
...
for chrom in RANGE:
    ...
    mmc = MMC[chrom]
    outfile.write('%s,%s,%d,%f,%d,%s,%f\n' %
                  ("MMC", bands[chrom], mmc, np.mean(dist), ITERATIONS, '0.05', pval[-1]))
for chrom in RANGE:
    dist = []
    dist = expected[chrom]
    ...
    deb = DEB[chrom]
    outfile.write('%s,%s,%d,%f,%d,%s,%f\n' %
                  ("DEB", bands[chrom], deb, np.mean(dist), ITERATIONS, '0.05', pval[-1]))
for chrom in RANGE:
    dist = []
    dist = expected[chrom]
    dist.append(Total[chrom])
    z = stats.zscore(dist)
    pval = stats.norm.sf(z)
    total = Total[chrom]
    outfile.write('%s,%s,%d,%f,%d,%s,%f\n' %
                  ("Total", bands[chrom], total, np.mean(dist), ITERATIONS, '0.05', pval[-1]))
outfile.close()
